  ...
  const allDates =eachDayOfInterval({
    ...
    const data =allDates.map(date=>{
        return{
            ...
            totalSales:bookings.filter(el=>isSameDay(date,new Date(el.created_at))).reduce((acc,cur)=>acc + cur.totalPrice,0),
            extrasSales:bookings.filter(el=>isSameDay(date,new Date(el.created_at))).reduce((acc,cur)=>acc + cur.extrasPrice,0),
        }
    })
